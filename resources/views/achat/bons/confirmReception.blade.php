...
<div class="container mx-auto md:px-36 py-12">
    <div class="bg-white shadow-md rounded-lg p-6">
        <h1 class="text-2xl font-bold text-gray-800 mb-4 text-center uppercase">Confirm le bon de reception N° <span class="text-black  rounded border bg-gray-200 px-1 shadow ">{{ $saleLines[0]->no_bl }}</span> / de {{ $saleLines[0]->date }}</h1>
        <div class=" relative overflow-hidden bg-gray-100 flex flex-wrap gap-4 items-center border border-gray-300 p-4 rounded-md">
            <span> <b class="text-lg">Fournisseur</b> : <span class="text-black rounded text-lg p-1 font-bold border bg-gray-200 px-1 shadow ">{{ $client->raison }} </span></span>
        </div>
        <form action="{{ route('confirmReception.save', $saleLines[0]->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
